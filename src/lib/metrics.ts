...
async function fetchAll<T>(
  table: string,
  select: string,
  filter: { col: string; op: "gte"; val: string }
) {
  let from = 0;
  let all: T[] = [];
  while (true) {
...
export async function getMetrics(): Promise<Metrics> {
  ...
  const [jobsFact, rawJobs, areas] = await Promise.all([
    ...
    fetchAll<any>("raw_jobs", "published_date,occupation_area_id_source", {
      col: "published_date",
      op: "gte",
      val: since,
    }),
  ...
  for (const j of jobsFact) {
    ...
    const wd = new Date(j.published_at).toLocaleDateString("en-US", {
      weekday: "short",
    });
  ...
  return {
    ...
    dailyJobs: Array.from(dailyJobs.entries()).sort((a, b) =>
      a[0] < b[0] ? -1 : 1
    ),
